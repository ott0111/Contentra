...
Clients may persist a non-sensitive workspace selection. They must not persist server credentials, database credentials, Paddle API/webhook secrets, Gemini keys, OAuth refresh tokens, encryption keys, or session tokens. A workspace header provides context only; server membership validation remains authoritative.
