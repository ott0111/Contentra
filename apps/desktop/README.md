...
The desktop UI uses only HTTP API calls with the browser session cookie and a locally stored workspace ID. It does not ship database, Stripe, Gemini, OAuth, encryption, or session secrets. OAuth and billing are opened only from API-issued flows; their completion is confirmed by refreshing API state.
