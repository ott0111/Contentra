    ...
    async (request, reply) => {
      ...
      try {
        ...
        try {
          ...
          request.log.warn({ err: error, workspaceId: ctx.workspaceId }, "onboarding audit log failed");
        }
        ...
        if (input.completed) {
          try {
            ...
            request.log.warn({ err: error, workspaceId: ctx.workspaceId }, "onboarding referral completion failed");
          }
        }
        ...
        request.log.error({ err: error, workspaceId: ctx.workspaceId }, "onboarding save failed");
        if (error instanceof ZodError) {
          return fail(reply, 400, "VALIDATION_ERROR", "One or more onboarding fields are invalid.", error.flatten());
        }
        return fail(reply, 500, "ONBOARDING_SAVE_FAILED", "We could not save your onboarding progress.");
      }
    },
  );
