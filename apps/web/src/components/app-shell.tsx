...
export function AppShell({children,active}:{children:React.ReactNode;active:string}){
 ...
 return <div className="app-shell">
  ...
  <header className="topbar">
   ...
   <nav className={`nav-pill ${mobileOpen?'mobile-open':''}`} aria-label="Main navigation">{nav.map(([label,href])=><Link onClick={()=>setMobileOpen(false)} className={active===label?'active':''} key={href} href={href}>{label}</Link>)}</nav>
